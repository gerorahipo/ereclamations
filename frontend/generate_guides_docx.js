...
// Logo absolute path for Word document importing
const logoPath = path.join(__dirname, 'public/logo-cnps.png').replace(/\\/g, '/');

// ─── TEMPLATE HTML POUR WORD ────────────────────────────────────────────────
const getBaseHtmlForDocx = (title, subtitle, badgeText, content) => `
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
<body>
  <!-- Page de garde -->
  <div style="padding: 40px; border: 12px solid #0f592f; background-color: #f8fafc; min-height: 600px;">
    <table border="0" style="border: none; margin-bottom: 80px;">
      <tr style="border: none;">
        ...
        <td width="240" align="right" style="border: none; text-align: right;">
          ${logoPath ? `<img src="${logoPath}" alt="Logo CNPS" width="90" />` : ''}
        </td>
      </tr>
    </table>
    ...
    <div style="margin-top: 140px; border-top: 1px solid #cbd5e1; padding-top: 20px;">
      <table border="0" style="border: none;">
        <tr style="border: none;">
          <td width="300" style="border: none; font-size: 9pt; color: #64748b; padding: 0;">
            <strong>Application eRéclamations</strong><br/>
            © 2026 CNPS Côte d'Ivoire. Tous droits réservés.
          </td>
          <td width="300" align="right" style="border: none; font-size: 9pt; color: #64748b; text-align: right; padding: 0;">
...
// ─── CONTENUS DES GUIDES (REUTILISES DU SCRIPT PRINCIPAL) ────────────────────
const agentContent = `
...
// ─── CONFIGURATION DES DOCUMENTS WORD ────────────────────────────────────────
const guides = [
  {
    role: 'agent',
...
async function generateAllDocx() {
  console.log("Démarrage de la génération des guides Word (.docx)...");
  ...
  for (const guide of guides) {
    ...
    const htmlContent = getBaseHtmlForDocx(
      guide.title, 
      guide.subtitle, 
      guide.badge, 
      guide.content
    ...
    try {
      const fileBuffer = await HTMLtoDOCX(htmlContent, null, {
        ...
        margins: {
          ...
          bottom: 1440,
          left: 1152,   // 0.8 inch = 1152 twips
          right: 1152
        },
        title: `Guide Utilisateur CNPS - ${guide.title}`,
        description: guide.subtitle,
        creator: 'Caisse Nationale de Prévoyance Sociale (CNPS) Côte d\'Ivoire',
        header: true,
        footer: true,
        headerType: 'default',
        footerType: 'default',
      ...
      const docxFilePath = path.join(outputDir, `guide_${guide.role}.docx`);
      fs.writeFileSync(docxFilePath, fileBuffer);
      console.log(`Document Word généré avec succès : ${docxFilePath}`);
      const stats = fs.statSync(docxFilePath);
      console.log(`Taille du fichier : ${(stats.size / 1024).toFixed(1)} Ko`);
    } catch (error) {
      console.error(`Erreur lors de la génération du document Word pour ${guide.role}:`, error);
    }
  }

  console.log(`\n==================================================`);
  console.log(`GÉNÉRATION WORD TERMINÉE ! Tous les guides .docx ont été générés dans le dossier :\n${outputDir}`);
}

generateAllDocx().catch(err => {
  console.error("Erreur fatale lors de la génération Word :", err);
});
